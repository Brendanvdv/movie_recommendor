...
#content
def build_content_sim(vectorized_movie_data):
...
#style
def build_review_style_sim(vectorized_movie_data):
...
def load_or_create_similarity_matrices(vectorized_data, pickle_path="similarity_matrices.pkl"):
    ...
    if os.path.exists(pickle_path):
        ...
        score_sim = build_review_score_sim(vectorized_data)
    ...
    return content_sim, style_sim, score_sim
#Query--------------------------------------------------------------------------------------------------

def hybrid_score(content_sim,review_sim,score_sim,alpha=0.4, beta=0.3, gamma=0.3):
    
    hybrid_sim = (alpha * content_sim) + (beta * review_sim) + (gamma * score_sim)
...
def main():
    ...
    # overlaps = analyze_critic_overlap(vectorized_data)
  
    content_sim, style_sim, score_sim = load_or_create_similarity_matrices(vectorized_data)
    hybrid_sim = hybrid_score(content_sim, style_sim, score_sim, alpha=0.5, beta=0.4, gamma=0.1)
